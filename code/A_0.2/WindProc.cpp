...
LRESULT CALLBACK BinApp::WindProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_DESTROY:
		...
		return 0;
	case WM_COMMAND:
		this->Command(LOWORD(wParam),wParam, lParam);
		return 0;
	case WM_MOVE:
		this->onMove();
		break;
	...
	case WM_SETCURSOR:
		...
		if (wind == this->CloseButtonHwnd || wind == this->MiniMizeButtonHwnd || wind == this->TranslateButtonHwnd || wind == this->hWndComboBox || wind == this->hWndComboBox2){
